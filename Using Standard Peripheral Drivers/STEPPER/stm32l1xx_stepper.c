/**
 ...
 ===============================================================================
    (#) Fill stepper struct: stepper_struct stepper1; 
				...
				It is esential to fill all these structure variables: (example)
					motor_pin_1_bank = GPIOA;
					motor_pin_1 = GPIO_Pin_4;
					motor_pin_2_bank = GPIOA;
					motor_pin_2 = GPIO_Pin_5;
					// omit these if 2 pin motor is used.
...
void timer3_init()
{
	...
	NVIC_InitTypeDef timer3_int;
	uint32_t timer_freq;
	uint32_t timer_prescaler;
	uint32_t timer_periode;
		
	RCC_GetClocksFreq(&system_freq);	//get system clocks
	timer_freq = system_freq.PCLK1_Frequency;	//TIM3 clock 
	timer_prescaler = TIM3_INCREMENT_RESOLUTION * (timer_freq / 1000000);	// example: Prescaler = 250[us] * 32[MHz] = 8000; 
	timer_periode = (1000000 / DEFAULT_PPS) / TIM3_INCREMENT_RESOLUTION;	// default pulses per second
	// 1000000/pulses_per_second
	
	RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM3, ENABLE);	//check smt32l1xx_rcc.c
	timer3.TIM_Prescaler = timer_prescaler;
	timer3.TIM_CounterMode = TIM_CounterMode_Up;
	timer3.TIM_Period = timer_periode -1;						
	timer3.TIM_ClockDivision = TIM_CKD_DIV1;
	TIM_TimeBaseInit(TIM3, &timer3);
	
	timer3_int.NVIC_IRQChannel = TIM3_IRQn;
	timer3_int.NVIC_IRQChannelPreemptionPriority = 10;
	timer3_int.NVIC_IRQChannelSubPriority = 0;
	timer3_int.NVIC_IRQChannelCmd = ENABLE;
	NVIC_Init(&timer3_int);

	TIM_ClearITPendingBit(TIM3, TIM_IT_Update);
	TIM_ITConfig(TIM3, TIM_IT_Update, ENABLE);	//check smt32l1xx_tim.c
...
{
  uint32_t period = (1000000 / pulsesPerSecond) / TIM3_INCREMENT_RESOLUTION;
